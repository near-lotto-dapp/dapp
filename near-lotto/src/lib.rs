...
// 1. СТВОРЮЄМО СТРУКТУРУ ДЛЯ ІСТОРІЇ
#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[borsh(crate = "near_sdk::borsh")]
#[serde(crate = "near_sdk::serde")]
pub struct DrawRecord {
    ...
    pub amount: String,
    pub timestamp_ms: u64, // Блокчейн сам дасть нам точний UTC час
}
...
pub struct LotteryPool {
    pub next_draw_time_ms: u64, // Коли наступний розіграш
    pub draw_period_ms: u64,    // Період (наприклад, 24 години)
    pub owner_id: AccountId,
    pub tickets: Vec<AccountId>,
    pub total_pool: NearToken,
    pub last_winner: Option<WinnerInfo>,

    // 2. ДОДАЄМО НОВІ ПОЛЯ В БАЗУ ДАНИХ КОНТРАКТУ
...
impl LotteryPool {
    ...
    pub fn new(owner_id: AccountId) -> Self {
        // 24 години у мілісекундах: 24 * 60 * 60 * 1000 = 86400000
        ...
        Self {
            ...
            history: Vec::new(),
            // Встановлюємо перший дедлайн
            next_draw_time_ms: env::block_timestamp_ms() + draw_period_ms,
            draw_period_ms,
        }
    }

    // ... (buy_tickets залишається без змін) ...
    ...
    pub fn draw_winner(&mut self) {
        ...
        // --- НОВА ЛОГІКА: ОНОВЛЮЄМО СТАТИСТИКУ ТА ІСТОРІЮ ---
        ...
        self.history.push(DrawRecord {
            ...
            amount: winner_prize_yocto.to_string(),
            timestamp_ms: env::block_timestamp_ms(), // Беремо час напряму з блокчейну!
        });
        ...
        // НОВЕ: Перезапускаємо таймер на наступні 24 години
        self.next_draw_time_ms = env::block_timestamp_ms() + self.draw_period_ms;
    }

    // --- НОВІ ФУНКЦІЇ ДЛЯ ФРОНТЕНДУ ---

    // Повертає: (Всього виграно, Всього зароблено на комісії) у форматі yoctoNEAR
    ...
    // Повертає масив всієї історії у форматі JSON
    ...
    // ПОВЕРТАЄМО JSON РЯДОК - це прибере помилку JsonSchema!
    ...
    pub fn unsafe_reinit(&mut self, owner_id: AccountId) {
        // Тільки власник може скинути контракт
        self.assert_owner();

        // Перезаписуємо все
        self.owner_id = owner_id;
        self.tickets = Vec::new();
        self.total_pool = NearToken::from_yoctonear(0);
        self.last_winner = None;

        env::log_str("Контракт успішно переініціалізовано!");
    }
}